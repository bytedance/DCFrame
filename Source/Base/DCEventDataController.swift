...
final public class DCEventID: DCUniqueID {
    // nothing
}

/// Define unique identifier of Shared Data
final public class DCSharedDataID: DCUniqueID {
    // nothing
}

public class DCUniqueID: Equatable {
    private static var globalID = DCProtector<Int64>(0)
    private(set) var ID: Int64
    public lazy var tag = ""
    public init() {
        DCUniqueID.globalID.write {
            $0 += 1
        }
        ID = DCUniqueID.globalID.directValue
    }
    ...
    public static func ==(lhs: DCUniqueID, rhs: DCUniqueID) -> Bool {
        return lhs.ID == rhs.ID
    }
}

/// Base class for chainable subscribe action
public class DCSubscribeAndable {
...
final public class DCSubscribeDataAndable: DCSubscribeAndable {
    @discardableResult
    public func and(_ sd: DCSharedDataID, completion: @escaping (Any?) -> Void) -> DCSubscribeDataAndable {
        return edc.subscribeData(sd, target: target, completion: completion)
    }
    
    @discardableResult
    public func and<T>(_ sd: DCSharedDataID, completion: @escaping (T) -> Void) -> DCSubscribeDataAndable {
        return edc.subscribeData(sd, target: target, completion: completion)
    }
    
    @discardableResult
    public func and<T>(_ sd: DCSharedDataID, completion: @escaping (T) -> Void, emptyCall: @escaping () -> Void) -> DCSubscribeDataAndable {
        return edc.subscribeData(sd, target: target, completion: completion, emptyCall: emptyCall)
    }
}

public typealias EDCSharedDataCallback = (() -> Any?)
...
final public class DCEventDataController: NSObject {
    ...
    private lazy var p_parentEDC = DCProtector<EDCWeakEventDataController>(EDCWeakEventDataController())
    
    private lazy var childsEDCList = DCProtector<[EDCChildItem]>([EDCChildItem]())
    private lazy var dataDict = DCProtector<[Int64: Any]>([Int64: Any]())
    
    private lazy var subscribeDataDict = EDCEventDataDict()
    private lazy var subscribeEventDict = EDCEventDataDict()
    
    private lazy var handleChildEDCQueue = DispatchQueue(label: "com.dcframe.DCEventDataController.handleChildEDCQueue", qos: .default)
    ...
    public func sharedData(of sd: DCSharedDataID) -> Any? {
        ...
        if let _data = data {
            if let dataCallBack = _data as? EDCSharedDataCallback {
                return dataCallBack()
            } else {
                return _data
            }
    ...
    @discardableResult
    public func subscribeData(_ sd: DCSharedDataID, target: NSObject, completion: @escaping (Any?) -> Void) -> DCSubscribeDataAndable {
        completion(sharedData(of: sd))
        subscribeDataDict.addItem(uniqueID: sd, target: target) { (data) in
            completion(data)
        }
        return DCSubscribeDataAndable(edc: self, target: target)
    }
    ...
    /// - Returns: Chainable for repetitive data subscription
    @discardableResult
    public func subscribeData<T>(_ sd: DCSharedDataID, target: NSObject, completion: @escaping (T) -> Void) -> DCSubscribeDataAndable {
    ...
    /// - Returns: Chainable for repetitive data subscription
    @discardableResult
    public func subscribeData<T>(_ sd: DCSharedDataID, target: NSObject, completion: @escaping (T) -> Void, emptyCall: (() -> Void)? = nil) -> DCSubscribeDataAndable {
    ...
    final private class EDCEventDataDict {
        private lazy var dict = DCProtector<[Int64: [EDCDataEventItem]]>([Int64: [EDCDataEventItem]]())
        private lazy var handleItemQueue = DispatchQueue(label: "com.dcframe.EDCEventDataDict.handleItemQueue", qos: .default)
        
        func addItem(uniqueID: DCUniqueID, target: NSObject, completion: @escaping (Any?) -> Void) {
            dict.write {
        ...
        func needClear(with uniqueID: DCUniqueID) {
            handleItemQueue.async {
                self.dict.write {
                    if let items = $0[uniqueID.ID] {
                        var tmpItems = items
        ...
        func getItems(with uniqueID: DCUniqueID) -> [EDCDataEventItem] {
            return dict.read {
                var tmpItems: [EDCDataEventItem]
                if let items = $0[uniqueID.ID] {
                    tmpItems = items
